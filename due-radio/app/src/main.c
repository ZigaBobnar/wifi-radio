...
bool boot_setup()
{
    ...
    if (c_time == NULL)
    {
        lcd_write_upper_formatted(&lcd, "Time sync");
        lcd_write_lower_formatted(&lcd, " FAIL");
    ...
    {
        time = c_time;
        console_put_formatted("\t  Time synced, %i-%i-%i %i:%i:%i", time->year, time->month, time->day, time->hour, time->minutes, time->seconds);
    }
