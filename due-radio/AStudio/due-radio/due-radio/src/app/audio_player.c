...
void audio_player_ensure_buffered(void) {
	...
	console_put_formatted("APLAYER> Running: %i, last refill: %i, current time: %i, samples: %i", audio_player_running, last_refill, current_time_ms, audio_player_buffered_samples);

	if (audio_player_running && current_time_ms - last_refill > 400 && audio_player_buffered_samples < 2000) {
		//console_put_formatted("APLAYER> Refill needed... %i", audio_player_buffered_samples);
	//if (audio_player_running &&
		//	current_time_ms - last_refill > 200/*AUDIO_PLAYER_SAMPLE_REFILL_TIME_THRESHOLD */&&
			//audio_player_buffered_samples < 3000/*AUDIO_PLAYER_SAMPLE_REFILL_THRESHOLD*/) {
